...
import { Truck, CreditCard, Clock, Package, MapPin, Check } from "lucide-react";
      ...
      <div className="container py-6">
        ...
        <section className="mb-8">
          ...
          <div className="grid md:grid-cols-2 gap-4">
            {[
              { name: "СДЭК", desc: "Доставка до двери или пункта выдачи. 2-7 рабочих дней.", icon: "📦" },
              { name: "ПЭК", desc: "Для крупногабаритных грузов. 3-10 рабочих дней.", icon: "🚛" },
              { name: "Деловые Линии", desc: "Экономичная доставка по всей России.", icon: "🚚" },
              { name: "Самовывоз", desc: "г. Москва, ул. Промышленная, 42. Пн-Пт 9:00-18:00.", icon: "🏢" },
            ].map((m, i) => (
              <div key={i} className="bg-card border border-border rounded-xl p-5 hover:shadow-card transition-shadow">
                <div className="flex items-start gap-3">
                  <div className="w-11 h-11 rounded-lg bg-secondary flex items-center justify-center text-xl shrink-0">{m.icon}</div>
                  <div>
